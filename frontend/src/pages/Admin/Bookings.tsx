...
export default function Bookings() {
  ...
  useEffect(() => { loadReservations(); loadSettings(); }, []);
      ...
      {showForm && (
        <form onSubmit={onSubmitForm} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-end gap-2">
            ...
            <div className="flex-1 min-w-[140px]">
              <label className="block text-xs font-medium text-slate-600 mb-1">Table ID</label>
              <input className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.tableId} onChange={e => setForm(f => ({ ...f, tableId: e.target.value }))} placeholder="1" />
            </div>
